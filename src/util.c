...
ArrayHandle filter_path(const char *path, int isfile)//replaces back slashes with slashes, adds trailing slash if missing, as ArrayHandle
{
	ArrayHandle path2;
	...
	if(!isfile&&path2->data[path2->count-1]!='/')//ensure trailing slash
		STR_APPEND(path2, "/", 1, 1);
	if(isfile&&path2->data[path2->count-1]=='/')//ensure trailing slash
	{
		path2->data[path2->count-1]=0;
		--path2->count;
	}
	return path2;
}
...
ArrayHandle get_filenames(const char *path, const char **extensions, int extCount, int fullyqualified)
{
	...
	for(;success=FindNextFileA(hSearch, &data);)
	{
		...
		if(!(data.dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY))
		{
			...
			if(!extCount||found)//return all files if no extensions provided
			{
